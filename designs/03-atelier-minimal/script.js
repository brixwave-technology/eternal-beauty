...
(function () {
  ...
  $("#services").innerHTML = D.services.map((s) => `
    <div class="row reveal" role="button" tabindex="0" aria-expanded="false" aria-controls="d-${s.id}">
      ...
      <div class="row__desc" id="d-${s.id}"><div><p>${esc(s.description)}</p></div></div>
    </div>`).join("");
  $$(".row").forEach((r) => {
  ...
  $$(".services__table, .works__grid, .process__list, .voices__row, .faq__list, .about__creds, #story").forEach((g) => $$(".reveal", g).forEach((el, i) => (el.style.transitionDelay = Math.min(i * 60, 360) + "ms")));
